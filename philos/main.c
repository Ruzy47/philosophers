...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/09/22 19:30:22 by rugrigor          #+#    #+#             */
/*   Updated: 2023/09/22 19:50:04 by rugrigor         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int fork_create(t_menu *menu)
{
	...
	while (++i < menu->philo_count)
		if (pthread_mutex_init(&menu->fork[i], NULL) != 0)
			return (1);
		//printf("aaa\n");
	return (0);
}
...
void	philo_init(t_menu *menu)
{
	...
	{
		menu->philo[i].num = i;
		menu->philo[i].right_fork = &menu->fork[i];
        if (i == menu->philo_count)
            menu->philo[i].left_fork = &menu->fork[1];
        else
            menu->philo[i].left_fork = &menu->fork[i + 1];
...
int main(int argc, char **argv)
{
	...
	philo_init(&menu);
	//printf("aaa\n");
	if (fork_create(&menu) != 0 || philo_create(&menu, &philo) != 0)
		return (write(2, "Ruzik_ERROR\n", 13));
	//	printf("aaa\n");
